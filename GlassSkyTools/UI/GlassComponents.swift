...
struct GlassCard<Content: View>: View {
    ...
    var body: some View {
        ...
        ZStack(alignment: .top) {
            ...
            // 顶部折射光条
            Capsule()
                .fill(LinearGradient(colors: [Color.clear, refLight, Color.clear], startPoint: .top, endPoint: .bottom))
...
struct GlassCardTitle: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            Circle()
                .fill(LinearGradient(colors: [.ambientViolet, .ambientCyan], startPoint: .top, endPoint: .bottom))
    ...
    var body: some View {
        Button(action: onClick) {
            Text(loading ? "处理中…" : text)
                ...
                .padding(.vertical, 13)
                .background(brush)
                .overlay(Capsule().stroke(LinearGradient(colors: [.white.opacity(0.45), .white.opacity(0.08)], startPoint: .top, endPoint: .bottom), lineWidth: 1))
                .clipShape(Capsule())
...
struct GlassOutlineButton: View {
    ...
    let onClick: () -> Void
    var enabled: Bool = true
    var brush: LinearGradient = LinearGradient(colors: [.white.opacity(0.22), .white.opacity(0.06)], startPoint: .top, endPoint: .bottom)
    ...
    var body: some View {
        Button(action: onClick) {
            Text(text)
                ...
                .padding(.vertical, 12)
                .background(brush)
                .overlay(Capsule().stroke(LinearGradient(colors: [.white.opacity(0.38), .white.opacity(0.08)], startPoint: .top, endPoint: .bottom), lineWidth: 1))
                .clipShape(Capsule())
        }
        .disabled(!enabled)
        .buttonStyle(PressableStyle())
...
struct GlassFloatingHint: View {
    ...
    var body: some View {
        ...
        .overlay(Capsule().stroke(LinearGradient(colors: [color.opacity(0.6), .white.opacity(0.15)], startPoint: .top, endPoint: .bottom), lineWidth: 1))
        .clipShape(Capsule())
...
struct GlassBottomNav: View {
    ...
    var body: some View {
        ...
        .overlay(RoundedRectangle(cornerRadius: 26).stroke(
            isDay
                ? LinearGradient(colors: [Color(hex: 0xFF7AA8C8).opacity(0.55), Color(hex: 0xFF5A7898).opacity(0.30)], startPoint: .top, endPoint: .bottom)
                : LinearGradient(colors: [.white.opacity(0.30), .white.opacity(0.06)], startPoint: .top, endPoint: .bottom),
            lineWidth: 1))
...
struct NavItem: View {
    ...
    var body: some View {
        Button(action: onClick) {
            ...
            .padding(.vertical, 10)
            .background(selected
                        ? LinearGradient(colors: [.accentStart.opacity(0.85), .accentEnd.opacity(0.85)], startPoint: .top, endPoint: .bottom)
