            ...
            ScrollView {
                VStack(spacing: 16) {
                    ...
                    GlassOutlineButton(
                        ...
                        brush: LinearGradient(colors: [.danger.opacity(0.25), .danger.opacity(0.1)])
                    )
                    if let hint = hint {
                        Text(hint)
                            .font(.system(size: 12))
                        ...
                        GlassButton(text: "确定", onClick: {
                            ...
                            Task {
                                do {
                                    ...
                                    user = updated
                                    await MainActor.run { show("昵称已更新"); editingName = false }
                                } catch {
                                    await MainActor.run { show("更新失败:\(error.localizedDescription)", false) }
                                }
                            }
                        })
                    }
                        ...
                        GlassButton(text: "确定", onClick: {
                            ...
                            Task {
                                do {
                                    ...
                                    await MainActor.run { show("密码已修改"); editingPwd = false }
                                } catch {
                                    await MainActor.run { show("修改失败:\(error.localizedDescription)", false) }
                                }
                            }
                        })
                    }
    ...
    func activate(type: String, code: String) {
        ...
        Task {
            ...
            do {
                ...
                await MainActor.run { show(error.localizedDescription, false) }
            }
        }
    }
    ...
    func handleImage(_ img: UIImage) {
        ...
        Task {
            do {
                ...
                await MainActor.run { show("上传失败:\(error.localizedDescription)", false) }
            }
        }
    }
}
